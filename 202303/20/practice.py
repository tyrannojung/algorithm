...
# https://www.acmicpc.net/board/view/77253 참고해서 풀자
...
while True:
    ...
    if ch_puzzle:
        ...
        if row_num == n-1 and column_num == e-1:
            t_puzzle.append(bfs_p[2] + 1)
            continue

        if east < e and bfs_l[row_num][east] == 1:
            east_bfs_l = [arr[:] for arr in bfs_l]
            east_bfs_p = bfs_p[:]
        ...
        if north >= 0 and bfs_l[north][column_num] == 1:
            north_bfs_l = [arr[:] for arr in bfs_l]
            north_bfs_p = bfs_p[:]
        ...
        if west >= 0 and bfs_l[row_num][west] == 1:
            west_bfs_l = [arr[:] for arr in bfs_l]
            west_bfs_p = bfs_p[:]
        ...
        if south < n and bfs_l[south][column_num] == 1:
            south_bfs_l = [arr[:] for arr in bfs_l]
            south_bfs_p = bfs_p[:]
